...
```bash
uv run python -m lrauthor.room_ops.export.export_room --scan office-elliott
uv run python -m lrauthor.room_ops.compile.build_from_room --room /path/to/Room
```
...
`lrauthor.room_ops` does not start Blender.
