...
from lrauthor import PACKAGE_ROOT, REPO_ROOT, telemetry
from lrauthor.pipeline.measure import paths as config

LAUNCHER = PACKAGE_ROOT / "models" / "trellis" / "inference.py"
PROC_MODULE = "lrauthor.models.object_generation.generate"
...
def run_procedural_for_scan(
    ...
    *,
    ...
    if not dry_run and glbs:
        from lrauthor.models.object_generation.sim.pipeline import compile_directory
...
def run_for_scan(
    ...
    *,
    ...
    from lrauthor.pipeline.author.reconstruct import service as reconstructor

    if not reconstructor.using_service() and python is None:
        # Auto-install the configured backend regardless of entry point.
        try:
            from lrauthor.models.registry import gen3d_from_settings
