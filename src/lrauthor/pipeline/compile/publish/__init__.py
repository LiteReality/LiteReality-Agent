...
def viewable_room(context: RunContext):
    ...
    viewer serves that.
    """
    from lrauthor.room_ops import compress
...
def run(context: RunContext, options: dict) -> StageResult:
    ...
    rc, log = run_module(
        context, "lrauthor.room_ops.compile.build_from_room",
    ...
    if frames and context.capture_dir.is_dir():
        ...
        compare_rc, compare_log = run_module(
            context, "lrauthor.room_ops.rendering.room_render.render_vs_capture",
