...
colour and placement.
    ...
    uv run python -m lrauthor.pipeline.author.realism.entrypoint --scene <scene dir>
...
current directory is inside a package. The explicit spelling still works and still wins:
    ...
    ... -m lrauthor.pipeline.author.realism.entrypoint \
...
def surfaces_for(room: Path) -> list[str]:
    from lrauthor.room_ops.surfaces import surface_ids
...
- `fetch_material(query, name, color_hex, pattern_strength)` — for any PATTERNED surface (carpet,
  ...
  `from lrauthor.room_ops.procedural_materials import make` then `make("carpet"|"fabric"|"plaster"|
...
from lrauthor.agent.providers.claude import build_capability_server  # noqa: E402,F401
...
async def run(room: Path, surface_ref: Path, scan: Path, model: str, max_turns: int, profile: str = "base",
    ...
    from lrauthor.agent.tool_narration import describe_tools_line
    ...
    from lrauthor import REPO_ROOT as repo_root
