...
Needs a compiled `Room.glb` (build the room first) and `python-fcl`.
    ...
    python -m litereality_agent.pipeline.compile.qc.correct --room <dir>          # dry run: print the plan
    python -m litereality_agent.pipeline.compile.qc.correct --room <dir> --apply  # write the nudges into Room.py
"""
...
from litereality_agent.pipeline.compile.qc.fix import (
    FREE_STANDING,
    MAX_NUDGE,
    _anchored,
    _obb,
